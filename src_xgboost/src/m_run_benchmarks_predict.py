...
if __name__ == "__main__":
    ...
    from utils.prediction import xgb_model_predict
    ...
    parser.add_argument('-mc',
                        '--clusteredmodel',
                        type=str,
                        default="Clustered_XGB_Classifier.pkl",
                        help="provide clustered model")
    parser.add_argument('-mf',
                        '--fullmodel',
                        type=str,
                        default="Full_XGB_Classifier.pkl",
    ...
    for multiplier in [0.5, 1, 2, 5, 10]:
        ...
        rec_score, macro_f1score, positives, pred_time = xgb_model_predict(intel_flag, model_clustered, X_test, y_test, 'clustered.png')
        ...
        logger.info("=======> XGB prediction time for clustered data = %s", str(pred_time))

        logger.info("\n")

        logger.info("=======> Testing model_full on test data")
        rec_score, macro_f1score, positives, pred_time = xgb_model_predict(intel_flag, model_full, X_test, y_test, 'full.png')
        ...
        logger.info("=======> Total Positives Predicted = %s", str(positives))
        logger.info("=======> XGB prediction time for full data = %s", str(pred_time))
    ...
    plt.title('XGBoost ROC: Test-data-num-'+str(size))

    plt.legend(loc="lower right")
    if FLAGS.intel:
        plt.savefig("XGBoost_ROC_patch")
    else:
        plt.savefig("XGBoost_ROC_not_patch")
    plt.show()
