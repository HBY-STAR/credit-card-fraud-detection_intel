...
if __name__ == "__main__":
    ...
    import numpy as np
    np.random.seed(42)
    from utils.training import split_data, DBSCAN_Clustering, xgb_model_hyper
    ...
    MODEL_FILE1 = 'XGB_Classifier.pkl'
    ...
    param_grid = {'max_depth': [5, 10],'learning_rate': [0.01],
    ...
    xgb_model, train_time = xgb_model_hyper(X_train_clustered, y_train_clustered, param_grid,intel_flag)
    logger.info("=======> XGB training time for clustered data = %s", str(train_time))
    ...
    xgb_model_full, train_time = xgb_model_hyper(X_train, y_train, param_grid, intel_flag)
    logger.info("=======> XGB training time for full data = %s", str(train_time))
    MODEL_FILE = 'Full_' + MODEL_FILE1
    joblib.dump(xgb_model_full, MODEL_FILE)
    logger.info("\n\n\n\n")
